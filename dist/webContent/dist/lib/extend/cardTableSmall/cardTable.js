layui.define(['table', 'laypage', 'jquery', 'element', 'laytpl'], function (exports) {
    ...
    function getCheckedData(obj, elem) {
        ...
        var reElem = obj;
        // $(reElem).addClass('layui-table-click').siblings().removeClass('layui-table-click'); // 禁用选中效果,不好看
        item.id = reElem.id;
        if (item.id) {
    ...
    function createCard(linenum, item, index, toolbar) {
        ...
        var card =
            '<div type=card id=card_' + item.id + ' class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + index +
            '" data-number="1"> <div class="project-list-item">' + (!!item.image ? ' <img class="project-list-item-cover" src="' + item.image + '">' : '') +
            '<div class="project-list-item-body">'+ 
            // ' <h2>' + item.title + '</h2> ' +
            '<div class="project-list-item-text layui-text">' + item.remark + '</div>' +
            ' <div class="project-list-item-desc"> <span class="time">' + item.time + '</span>' + (!!template ? ' <div class="ew-head-list">' + template + '</div>' : '') + ' </div> </div > </div > </div > '
